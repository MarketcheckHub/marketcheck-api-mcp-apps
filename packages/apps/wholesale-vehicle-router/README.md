# Wholesale Vehicle Router ![Wholesaler](https://img.shields.io/badge/Wholesaler-78716c?style=flat-square)

![Screenshot](../../../static/screenshots/wholesale-vehicle-router.png)

## Overview

Batch VIN evaluation for wholesale operations. Decodes each VIN and predicts retail market value to determine optimal routing. Helps wholesalers decide which vehicles to retail, wholesale, or send to specific markets based on regional demand.

## Who Is This For

Wholesale vehicle buyers and routers

## MarketCheck API Endpoints Used

| Endpoint | Name | Docs |
|----------|------|------|
| `GET /v2/decode/car/neovin/{vin}/specs` | VIN Decode | [View docs](https://apidocs.marketcheck.com/#decode-vin) |
| `GET /v2/predict/car/us/marketcheck_price/comparables` | Price Prediction | [View docs](https://apidocs.marketcheck.com/#price-prediction) |

## Parameters

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `vins` | string | Yes | Comma-separated VINs |
| `zip` | string | Yes | Primary market ZIP |

## Derivative API Endpoint

**`POST https://apps.marketcheck.com/api/proxy/route-wholesale-vehicles`**

> This is a composite endpoint that orchestrates multiple MarketCheck API calls into a single response. It is provided for reference and experimentation purposes only and is not under LTS (Long-Term Support).

## How to Run

### Browser (standalone)

Open the app directly in a browser with your MarketCheck API key:

```
https://apps.marketcheck.com/app/wholesale-vehicle-router/?api_key=YOUR_API_KEY
```

### MCP (Model Context Protocol)

Add to your MCP client configuration (e.g. Claude Desktop):

```json
{
  "mcpServers": {
    "marketcheck": {
      "command": "npx",
      "args": [
        "-y",
        "@anthropic/marketcheck-mcp"
      ],
      "env": {
        "MARKETCHECK_API_KEY": "YOUR_API_KEY"
      }
    }
  }
}
```

### Embed (iframe)

Embed in any webpage:

```html
<iframe src="https://apps.marketcheck.com/app/wholesale-vehicle-router/?api_key=YOUR_API_KEY" width="100%" height="800" frameborder="0"></iframe>
```

## Limitations

- Demo mode shows mock data
- Requires MarketCheck API key for live data
- Browser-based — no server required for standalone use
- Data covers US market (95%+ of dealer inventory)

## Links

- [MarketCheck Developer Portal](https://developers.marketcheck.com)
- [API Documentation](https://apidocs.marketcheck.com)
- [Wholesale Vehicle Router App](https://apps.marketcheck.com/app/wholesale-vehicle-router/)
- [GitHub Repository](https://github.com/anthropics/marketcheck-mcp-apps)
